#!/usr/bin/env python3
"""
Test the improved BC model (v3) with randomized candidate order.

This script tests the BC model on new queries to evaluate its performance
after fixing the overfitting issue.
"""
...
class BCModelTester:
    """Tester for the improved BC model."""
    ...
    def generate_candidates_for_query(self, query: str) -> List[Dict[str, Any]]:
        """Generate candidate documents for a query with realistic scores."""
        # Create more realistic candidates with varied scores
        mock_docs = [
            {"doc_id": "1234", "content": f"Comprehensive guide about {query[:20]}...", "rrf_score": 0.9, "bm25_score": 0.8, "vector_score": 0.85},
            {"doc_id": "5678", "content": f"Detailed analysis of {query[:15]}...", "rrf_score": 0.7, "bm25_score": 0.9, "vector_score": 0.75},
            {"doc_id": "9012", "content": f"Expert insights on {query[:25]}...", "rrf_score": 0.8, "bm25_score": 0.6, "vector_score": 0.9},
            {"doc_id": "3456", "content": f"Basic information about {query[:18]}...", "rrf_score": 0.5, "bm25_score": 0.5, "vector_score": 0.6},
            {"doc_id": "7890", "content": f"Additional context for {query[:22]}...", "rrf_score": 0.6, "bm25_score": 0.7, "vector_score": 0.65},
        ]
        
        # Add some randomness but keep realistic score distributions
    ...
    def test_model(self) -> Dict[str, Any]:
        ...
        for i, query_data in enumerate(test_queries):
            ...
            selection_counts[doc_id] = selection_counts.get(doc_id, 0) + 1
        ...
        results['selection_distribution'] = selection_counts
    ...
    def print_summary(self, results: Dict[str, Any]):
        ...
        print("\n" + "="*60)
        print("BC MODEL V3 TESTING SUMMARY")
        print("="*60)
        ...
        for doc_id, count in results['selection_distribution'].items():
            percentage = (count / results['total_queries']) * 100
            print(f"  - Document {doc_id}: {count} times ({percentage:.1f}%)")
...
def main():
    ...
    # Save results
    output_path = "outputs/bc_model_v3/test_results_v3.json"
