#!/usr/bin/env python3
"""
Train Behavioral Cloning (BC) model with LLM expert trajectories - Version 3.

This version fixes the overfitting issue by randomizing candidate order during training,
so the model learns to select based on document features rather than position.
"""
...
class BCTrainer:
    ...
    def load_trajectories(self, trajectory_file: str) -> List[BCTrainingExample]:
        ...
        for traj in trajectories:
            ...
            for i, doc in enumerate(documents_selected):
                # Create candidates list with ALL documents from the trajectory
                candidates = documents_selected.copy()
                
                # Find the selected document
                selected_doc = doc
                
                # Randomize the order of candidates to prevent position bias
                random.shuffle(candidates)
                
                # Find the new index of the selected document after shuffling
                ...
                example = BCTrainingExample(
                    ...
                    selected_idx=selected_idx,  # Now the selected document is at a random position
    ...
    def save_model(self, output_dir: str):
        ...
        # Save model
        model_path = output_path / "bc_model_v3.pth"
        torch.save(self.model.state_dict(), model_path)
        
        # Save config
        config_path = output_path / "bc_config_v3.json"
...
def main():
    ...
    output_dir = "outputs/bc_model_v3"
    ...
    history_path = Path(output_dir) / "training_history_v3.json"
